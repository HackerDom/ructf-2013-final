...
//  ласс дл€ обновлени€ цены украденных флагов в таблицах
//   * flag_price (туда новые данные добавл€ютс€ - хранитс€ вс€ истори€ за все раунды) 
//   * teams (там данные о стоимости флагов замен€ютс€ - хранитс€ только стоимость за последний раунд) 
...
public class FlagPriceManager {
	...
	public FlagPriceManager(Connection conn) throws SQLException {
		psDel = conn.prepareStatement("DELETE FROM flag_price WHERE round = ?");											// ќчистить старые данные 
		psIns = conn.prepareStatement("INSERT INTO flag_price(round,time,team,price,rank,rank_def,rank_att) VALUES (?,NOW(),?,?,?,?,?)");
		psSel = conn.prepareStatement("SELECT team_id, privacy+availability, attack FROM rounds_cache WHERE round = ?");	// ѕрочитать актуальные данные о баллах
		psUpd = conn.prepareStatement("UPDATE teams SET flag_price = ? WHERE id = ?");										// ќбновить цену флага в таблице команд
	}
